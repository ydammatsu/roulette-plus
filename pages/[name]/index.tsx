...
const getInitialState = (rouletteName: string) => {
  try {
    return getAppStateFromLS(rouletteName);
  } catch {
    return initialState;
  }
};
...
const RoulettePage: NextPage = () => {
  ...
  useEffect(() => {
    const initialState = getInitialState(rouletteName)
    dispatch({
      name: rouletteName,
          ...
          {state.candidates.map((c, i) => {
            return (
              <CandidateRow
                ...
                deleteHandler={() => {
                  dispatch({ name: rouletteName, type: "delete_candidate", index: i });
                }}
                hideHandler={() => {
                  dispatch({ name: rouletteName, type: "hide_candidate", index: i });
                }}
                showHandler={() => {
                  dispatch({ name: rouletteName, type: "show_candidate", index: i });
                }}
              />
            );
          })}
